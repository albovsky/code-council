/**
 * `council quickstart` — fire a 30-second self-test review against
 * whatever CLI the user has on PATH and tail it inline.
 *
 * The activation moment. Telemetry post-launch showed 78% of installs
 * never fired a chat — the bottleneck is the gap between `council start`
 ...
 *   - No CLIs detected → point at `council connect` and `council diagnose`
 *   - Daemon not running → instruct `council start`
...
const SAMPLE_WORK =
  'Quickstart self-test for Code Council — does the reviewer catch the off-by-one in the average() loop?';
...
description: |
  Auto-generated by \`council quickstart\` — single reviewer slot routed
...
export async function runQuickstart(opts: QuickstartOptions = {}): Promise<void> {
  console.log('');
  console.log(`  ${sym.rocket} ${c.bold('Code Council quickstart')} ${c.dim('— 30-second self-test')}`);
  console.log('');
  ...
  if (!info) {
    ...
    console.log(`     run ${c.bold('council start')} first, then re-run quickstart`);
    process.exitCode = 1;
    return;
  }
  ...
  if (detected.length === 0) {
    ...
    console.log(`     then run ${c.bold('council connect')} to wire MCP`);
    console.log(`     ${c.gray('details:')} ${c.bold('council diagnose')}`);
    process.exitCode = 1;
    return;
  }
  ...
  if (!answer) {
    ...
    console.log('');
    console.log(`  ${c.gray('see')} ${c.bold('council diagnose')} ${c.gray('for failure context')}`);
    process.exitCode = 1;
    return;
  } else {
...
async function resolveCockpitUrlSafe(): Promise<string | null> {
  // Read the cockpit port out of `~/.code-council/daemon.json` rather than
